...
export default class GetFolderData {
  ...
  private async getData() {
    ...
    if (this.fileIsLinked) this.getPlaylist();
    else this.data!.playlist = null;
    await this.getMetas();
    this.sortItems();
  }
  ...
  private getLinkedFile() {
    try {
      ...
      } else this.data.currentDirectory = this.url || '/';
    } catch (e) {
  ...
  private getFolderList() {
    ...
    this.data.filesList = fs
      .readdirSync(`${this.contentPath}/${this.data!.currentDirectory}`, {
        withFileTypes: true,
      })
      .map((element) => {
        const name = element.name;
        const type = element.isDirectory() ? 'folder' : 'file';
        const ext = element.isDirectory()
          ? null
          : element.name.match(/([^.]+)$/g)![0];
        ...
        const birthTime = fs.statSync(
          `${this.contentPath}/${this.data!.currentDirectory}/${element.name}`,
        ).birthtime;
        return {
          name,
          type,
  ...
  private getAllPaths() {
    ...
    const abs = `/${this.contentPath}/${this.data!.currentDirectory}/`.replace(
      /\/{2,}/,
      '/',
    );
    const lvlUp = rel?.match(/.*\/(?=.+$)/)
      ? rel.match(/.*\/(?=.+$)/)![0]
      : null;
  ...
  private async getMetas() {
      ...
      for (let i = 0; i < this.data!.filesList!.length; i++)
        if (this.data!.filesList![i].ext === 'mp3')
          this.data!.filesList![i].metadata =
            await GetFolderData.getMetaDataFields(
    ...
    try {
      if (
        ...
        this.data!.linkedFile!.metadata = await GetFolderData.getMetaDataFields(
          `${this.contentPath}/${this.data!.linkedFile!.url}`,
        );
    } catch (e) {
    ...
    try {
      if (this.data!.playlist)
        for (const elem of this.data!.playlist)
          elem.metadata = await GetFolderData.getMetaDataFields(
            `${this.contentPath}/${decodeURI(elem.url)}`,
          );
    } catch (e) {
