...
export const omit = <T1 extends object, T2 extends keyof T1>(object: T1, keys: T2[]): Omit<T1, T2> =>
  Object.fromEntries(Object.entries(object).filter(([key]) => !keys.includes(key as T2))) as Omit<T1, T2>;

export const pick = <T1 extends object, T2 extends keyof T1>(object: T1, keys: T2[]): Pick<T1, T2> =>
